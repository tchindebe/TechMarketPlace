...
@section('content')
    ...
    <div class="flex items-center justify-between my-4 mb-4 text-sm font-semibold text-purple-100">
        ...
        <span class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Sponsored products') }} <span class="text-green-800 font-bold">({{$sponsored->count()}})</span>
        </span>
        </div>
    </div>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                    <span hidden>{{$sum = 0}}</span>
                    @foreach($sponsored as $sponsored)
                        <span hidden>{{$sum = $sum + $sponsored->prise}}</span>
                        <tr class="bg-white border-b text-gray-700 hover:bg-gray-50 ">
                            ...
                            <td class="px-6 py-4">
                                <img alt="image" class="img-responsive" style="width: 50px; height: 40px;" src="{{asset('storage') . '/' . $sponsored->images}}">
                            </td>
                            <td class="px-6 py-4">
                                {{ $sponsored->transaction_type }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $sponsored->prise }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $sponsored->created_at->format('D-M-Y') }}
                            </td>
                        </tr>
                    @endforeach
                    <tr class="bg-white border-b text-gray-700 hover:bg-gray-50 ">
                        <td colspan="6" class="px-6 py-4 text-blue-900 font-bold text-2xl">Total amount</td>
                        <td class="px-6 py-4 text-blue-900 font-bold text-2xl">{{$sum}} XAF</td>
                    </tr>
                    </tbody>
                </table>
            </div>
