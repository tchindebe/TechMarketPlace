...
@section('content')
    ...
    <div class="flex-1 p:2 sm:p-6 justify-between flex flex-col h-screen">
               ...
               <svg width="20" height="20">
                  <circle cx="8" cy="8" r="8" fill="currentColor"></circle>
                    ...
                    @if($chatUser->profile->logo)
                        <img src="{{asset('storage') . '/' . $chatUser->profile->logo}}" alt="username" class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">
                    @endif
                ...
                <div class="flex flex-col leading-tight">
                    <div class="text-2xl mt-1 flex items-center">
                        <span class="text-gray-700 mr-3">{{$chatUser->username}}</span>
                    </div>
                    <span class="text-lg text-gray-600">
                        {{$chatUser->user_type}}
                        @if(Cache::has('user-is-online-' . $chatUser->id))
                            <span class="text-green-600" style="font-size: 11px;">Online : {{ \Carbon\Carbon::parse($chatUser->last_seen)->diffForHumans() }}</span>
                        @else
                            <span class="text-red-600" style="font-size: 11px;">Off line :  {{ \Carbon\Carbon::parse($chatUser->last_seen)->diffForHumans() }}</span>
                        @endif
                    </span>
                </div>
            </div>
                ...
                @if($messages->user_id != Auth::User()->id)
                    <div class="chat-message">
                        <div class="flex items-end">
                            ...
                            @endif
                            @if($chatUser->profile->logo == null)
                                <img src="{{asset('assets/clientAsset/plugins/images/users/shop.png')}}" alt="username" class="w-6 h-6 rounded-full order-1">
                            @endif
                        </div>
                    </div>
                @else
                    <div class="chat-message">
                        <div class="flex items-end justify-end">
                            <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-1 items-end">
                                <div><span class="px-4 py-2 rounded-lg inline-block rounded-br-none bg-blue-600 text-white ">{{$messages->message}}</span></div>
                            </div>
        ...
        <div class="border-t-2 border-gray-200 px-4 pt-4 mb-2 sm:mb-0">
            <form action="{{route('admin.messenger.send', $chatUser->id)}}" method="post">
                @csrf
